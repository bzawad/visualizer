...
MiniCubeVisualizer::MiniCubeVisualizer() : aspectRatio(1.0f) {
}

void MiniCubeVisualizer::initialize(int width, int height) {
    // Force resolution to 128x43 (ignore passed parameters)
    (void)width;
    (void)height;
    Visualizer::initialize(128, 43);
    aspectRatio = static_cast<float>(128) / 43.0f;
...
void MiniCubeVisualizer::renderFrame(const std::vector<float>& audioData,
                               double* in,
                               fftw_complex* out,
                               fftw_plan& plan,
                               float timeSeconds) {
...
void MiniCubeVisualizer::renderLiveFrame(const std::vector<float>& audioData,
                                   double* in,
                                   fftw_complex* out,
                                   fftw_plan& plan,
...
std::vector<float> MiniCubeVisualizer::calculateMagnitudes(fftw_complex* out) {
...
void MiniCubeVisualizer::render(float time, const std::vector<float>& magnitudes) {
    ...
    if (peakMagnitude > 0.0f) {
        for (size_t i = AMPLITUDE_START_BIN; i < ampEndBin; i++) {
            ...
            currentAmplitude += normalizedMag * normalizedMag * 2.0f;
        }
    ...
    glLoadIdentity();
    glTranslatef(0.0f, 0.0f, -3.5f);  // Closer for mini viewport
...
void MiniCubeVisualizer::drawCube(float rotationAngle, float scale) {
    glLineWidth(LINE_WIDTH);
    
    // Set up the cube's position and rotation
    glPushMatrix();
    
    // Center the cube in world space (smaller offset for mini viewport)
    glTranslatef(0.0f, 0.2f, 0.0f);  // Move up slightly to center vertically
    ...
    // Draw the cube edges in monochrome green
    glBegin(GL_LINES);
    for (size_t i = 0; i < edges.size(); i += 2) {
        // Set color to monochrome green (bright green like mini racer)
